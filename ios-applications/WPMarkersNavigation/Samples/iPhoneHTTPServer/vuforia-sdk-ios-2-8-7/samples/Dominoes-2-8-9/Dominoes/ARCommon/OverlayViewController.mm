...
////STRIP_BEGIN
    ...
    if (MENU_OPTION_WANTED == swapCameraIx && qUtils.noOfCameras > 1)
    {
        swapCameraIx = [mainOptionsAS addButtonWithTitle:@"Swap camera"];
    }
#endif
////STRIP_END
...
////STRIP_BEGIN
#ifdef AR_PRIVATE_BUILD
    swapCameraIx = MENU_OPTION_WANTED;
#endif
////STRIP_END
...
+ (BOOL) doesOverlayHaveContent
{
    ...
    ////STRIP_BEGIN
#ifdef AR_PRIVATE_BUILD
    if ([QCARutils getInstance].noOfCameras > 1)
        count++;
#endif
    ////STRIP_END
...
////STRIP_BEGIN
#ifdef AR_PRIVATE_BUILD
    else if (swapCameraIx == buttonIndex)
    {
        [qUtils swapCamera];
    }
#endif
