...
////STRIP_BEGIN
        ...
        NSString *frontcamerapath = [cachesDirectory stringByAppendingPathComponent:@"QCAR/frontcamera"];
        if ([[NSFileManager defaultManager] fileExistsAtPath:frontcamerapath])
        {
            NSLog(@"QCARutils: forcing start up with front camera");
            activeCamera = QCAR::CameraDevice::CAMERA_FRONT;
        }
#endif 
////STRIP_END
        noOfCameras = 1;
...
- (void)swapCamera
{
    if (activeCamera == QCAR::CameraDevice::CAMERA_FRONT)
        activeCamera = QCAR::CameraDevice::CAMERA_BACK;
    else
        activeCamera = QCAR::CameraDevice::CAMERA_FRONT;
    ...
    if (!QCAR::CameraDevice::getInstance().init(activeCamera))
    {
        NSLog(@"Can't init the new camera to swap!");
        return;
    }
    ...
    {
        NSLog(@"Can't select the proper video mode!");
        return;
    }
    ...
    {
        NSLog(@"Can't start the new camera after swap!");
        return;
    }
