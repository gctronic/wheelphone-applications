...
import com.wheelphone.targetNavigation.WheelphoneTargetNavigation;
 ...
 */
public class WheelphoneTargetNavigation extends Activity implements OnSharedPreferenceChangeListener,  TextToSpeech.OnInitListener, WheelPhoneRobotListener {
	...
	public static final int DIST_DIFF_THR = 200; // Threshold of the difference in the distance detected when switching from a big to a small target.
	...
	private static final long DOCK_TIMEOUT = 40000;	// 40 seconds  
	private static final long TARGET_TIMEOUT = 30000;	// Timeout given in ms when going towards markers (not the docking); if passed then go to next target.
	...
	private int LINEAR_CONST = 100; //12;
	private int ANGULAR_CONST = 16; //8;
	private int mLeftSpeed;
	private int mRightSpeed;
	private int mCurrentTargetId=2;
	private int mStopDistance=120;	// distance (in scene units) used to know when we are near a target point and we want to pass to the next
	private int mStopDistanceTemp=120;
	private int mChargeTime=60;		// given in seconds (base time is usb communication of 50 ms)
...
//	private EditText txtRoomWidth, txtRoomLength;
	...
	private EditText txtTagLinearCoeff, txtTagRotationCoeff, txtDockBaseSpeed, txtSearchAngle;
...
//	int roomWidth, roomLength;
    ...
    private static final String NATIVE_LIB_SAMPLE = "WheelphoneTargetNavigation";
	...
	class trackerTask extends TimerTask {          
		@Override        
		public void run() {             
			WheelphoneTargetNavigation.this.runOnUiThread(new Runnable() {                  
				//@Override                 
				public void run() {
					getTrackInfo();
...
//					fpv.updateRobotInfo(0+(int)(Math.cos(angleTargetRobot[1]*Math.PI/180.0)*targetDist[1]*16.0/50.0), 100-(int)(Math.sin(angleTargetRobot[1]*Math.PI/180.0)*targetDist[1]*16.0/50.0), (int)currentAbsoluteOrientation);
    ...
    private class InitQCARTask extends AsyncTask<Void, Integer, Boolean>
    {
        ...
        protected Boolean doInBackground(Void... params)
        {
            ...
            {
                QCAR.setInitParameters(WheelphoneTargetNavigation.this, mQCARFlags);
        ...
        protected void onPostExecute(Boolean result)
        {
            ...
            else
            {
                ...
                AlertDialog dialogError = new AlertDialog.Builder(
                    WheelphoneTargetNavigation.this
                ).create();
...
//            menu.add(MENU_ITEM_ACTIVATE_CONT_AUTO_FOCUS);
   ...
   public void updateMarkersInfo(int markerId, boolean detected, int i1, int i2, float dist, float z, float tpx, float tpz) {
       ...
       logString += "detected=" + detected + "(" + markerId + ")" + ",Xscreen="+targetX[markerId] + ",Yscreen="+targetY[markerId] + ",orient="+targetOrientation[markerId] + ",dist="+targetDist[markerId] + ",Xpose="+targetPoseX[markerId] + ",Zpose="+targetPoseZ[markerId] + ",angle="+angleTargetRobot[markerId];
	   	...
	   	if(!detected) {	
		    ...
		    if((targetDist[markerId] - targetDistPrev[markerId]) > DIST_DIFF_THR) {	// Detected the small target just after the big one (passing from big to small target).
		       switchToLittleTag[markerId] = true;
		    } else if((targetDist[markerId] - targetDistPrev[markerId]) < -DIST_DIFF_THR) {	// Detected the big target just after the small one (passing from small to big target).
	   	...
	   	if(markerId >= 2) {	// Only for targets that aren't on docking station.
		   	if(markerId==mCurrentTargetId) { // Considering the same target as before.
		   		if(targetDetectedInfo[markerId]==NO_TARGET_FOUND) {	// Target lost somehow.
		   			if((System.currentTimeMillis() - mTimestampLastSeen > 500) && mSeenOnce==true) { // Just lost the target, wait a little to stabilize image.
		   				mIsExploring = true;			
		   			    if(lastRotation == ROT_LEFT) {	// Explore, maintaining last rotation accomplished. 
		   			    	mDesiredRotation = -4;
		   			    } else {
		   			    	mDesiredRotation = 4;
		   			    }
		   				mDesiredAcceleration = 0; // Do a pivot rotation.
		   			}
		   			if (System.currentTimeMillis() - mTimestampLastSeen > TARGET_TIMEOUT && globalState!=STATE_WAITING_START) {	// Target completely lost, switch to the next.
		   				startExploring();
		   				goToNextTarget();		   				
		   			}
		   		} else { // Target is/was visible, advance while trying to reaching it by rotating based on target X screen coordinate.
		   			mSeenOnce = true;
		   			mIsExploring = false;
					// Set the rotation on a range between -1 (when target detected completely on the left) and 1 (target completely on the right);
		   			// these are the maximum ranges for the angular speed used to compute the final speed.
   ...
   public void onNativeCrashed() {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        wl = pm.newWakeLock(PowerManager.SCREEN_DIM_WAKE_LOCK, "com.wheelphone.targetNavigation.wakelock");
...
//        });
        ...
        txtTagLinearCoeff = (EditText) findViewById(R.id.tagLinearCoeff);
        txtTagLinearCoeff.setOnEditorActionListener(new OnEditorActionListener() {        	
            public boolean onEditorAction(TextView v, int actionId, KeyEvent event) {
                boolean handled = false;
                if (actionId == EditorInfo.IME_ACTION_DONE) {               	
                	LINEAR_CONST = Integer.parseInt(v.getText().toString());
                    InputMethodManager imm = (InputMethodManager)v.getContext().getSystemService(Context.INPUT_METHOD_SERVICE);
                    imm.hideSoftInputFromWindow(v.getWindowToken(), 0);                     
                    handled = true;
                }
        ...
        txtTagRotationCoeff = (EditText) findViewById(R.id.tagRotationCoeff);
        txtTagRotationCoeff.setOnEditorActionListener(new OnEditorActionListener() {        	
            public boolean onEditorAction(TextView v, int actionId, KeyEvent event) {
                boolean handled = false;
                if (actionId == EditorInfo.IME_ACTION_DONE) {               	
                	ANGULAR_CONST = Integer.parseInt(v.getText().toString());
                    InputMethodManager imm = (InputMethodManager)v.getContext().getSystemService(Context.INPUT_METHOD_SERVICE);
                    imm.hideSoftInputFromWindow(v.getWindowToken(), 0);                     
                    handled = true;
                }
        ...
        txtDockBaseSpeed = (EditText) findViewById(R.id.dockBaseSpeed);
        txtDockBaseSpeed.setOnEditorActionListener(new OnEditorActionListener() {        	
            public boolean onEditorAction(TextView v, int actionId, KeyEvent event) {
                boolean handled = false;
                if (actionId == EditorInfo.IME_ACTION_DONE) {               	
                	BASE_SPEED = Integer.parseInt(v.getText().toString());
                    InputMethodManager imm = (InputMethodManager)v.getContext().getSystemService(Context.INPUT_METHOD_SERVICE);
                    imm.hideSoftInputFromWindow(v.getWindowToken(), 0);                     
                    handled = true;
                }
        ...
        txtSearchAngle = (EditText) findViewById(R.id.searchAngle);
        txtSearchAngle.setOnEditorActionListener(new OnEditorActionListener() {        	
            public boolean onEditorAction(TextView v, int actionId, KeyEvent event) {
                boolean handled = false;
                if (actionId == EditorInfo.IME_ACTION_DONE) {               	
                	mSearchAngle = Integer.parseInt(v.getText().toString());
                    InputMethodManager imm = (InputMethodManager)v.getContext().getSystemService(Context.INPUT_METHOD_SERVICE);
                    imm.hideSoftInputFromWindow(v.getWindowToken(), 0);                     
                    handled = true;
                }
		...
		if(globalState == STATE_WAITING_START) {
		    ...
		    if(mIsExploring) {		    	
		    	if(rotationInProgress) {
		    		if(Math.abs((mOdomTheta/Math.PI*180.0)) >= mSearchAngle) {			    	
				    	lSpeed = 0;
				    	rSpeed = 0;
				    	rotCounter = 0;
				    	rotationInProgress = false;
		    		...
		    		rotCounter++;
		    		if(rotCounter >= 28) {	// stopped for about 1.5 seconds
		    ...
		    if(chargeStatus == ROBOT_NOT_CHARGING) {
		    	...
		    	chargeCounter++;
		    	if(chargeCounter == 20) { // Contact is stable.
		    		instableChargeCounter = 0;
		    		tts.speak("charging", TextToSpeech.QUEUE_FLUSH, null);
		    	} else if(chargeCounter >= (mChargeTime*20)) { // "chargeCounter" increments based on USB comm. (20 Hz).
		    		mGlobalTimestamp = System.currentTimeMillis();
		    		mTimestampDocking = System.currentTimeMillis();
		    		chargeCounter = mChargeTime*20;
		    		//if(mContinuousMode && (wheelphone.getBatteryCharge()>=70)) { // Wait the battery is at least 70% before starting next tour.
		    		if(mContinuousMode) {
			    		dockReachedTimeout = 0;
						lSpeed = -40;
						rSpeed = -40;
			...
			dockReachedTimeout++;
			if(dockReachedTimeout >= 40) {	// Go back for about 2 seconds.
				lSpeed = 0;
				rSpeed = ROT_SPEED;
				globalState = STATE_ROTATE_FOR_SCREENSHOT;
		    ...
		    txtConnected.setTextColor(getResources().getColor(R.color.red));	
		    rotCounter++;
			if(rotCounter>=20) {	// Wait for the image to be stable before taking the picture.
...
//		}
		...
		mLeftSpeed = (int)(linearAcc*LINEAR_CONST + angularAcc*ANGULAR_CONST);

		mRightSpeed = (int)(linearAcc*LINEAR_CONST - angularAcc*ANGULAR_CONST);
    ...
    public void resetBehavior(View view) {
    	if(globalState == STATE_ROBOT_CHARGING) {
    		dockReachedTimeout = 0;
			lSpeed = -40;
			rSpeed = -40;
    		globalState = STATE_ROBOT_GO_BACK_TARGET;
